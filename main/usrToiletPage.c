...
static const char *s_tag = "usrToiletPage";
static usrToiletPage_t buttonPage = {0};
...
{
    return usrSettingsPage_getButtonName(0, index); // 0 = Toilet section
}
...
// Color Palette - NIGHT MODE (Dark theme from image)
#define COLOR_BG_MAIN_NIGHT lv_color_make(8, 15, 22)           // Very dark background
#define COLOR_BG_PANEL_NIGHT lv_color_make(12, 20, 28)         // Panel background
#define COLOR_NEON_CYAN lv_color_make(0, 210, 255)             // Neon cyan
#define COLOR_NEON_BLUE lv_color_make(0, 163, 224)             // Electric blue
#define COLOR_TEXT_PRIMARY_NIGHT lv_color_make(255, 255, 255)  // White
#define COLOR_TEXT_SECONDARY_NIGHT lv_color_make(138, 155, 179) // Gray-blue
#define COLOR_SIDEBAR_BG_NIGHT lv_color_make(10, 18, 26)       // Sidebar background
#define COLOR_CARD_BG_NIGHT lv_color_make(15, 25, 35)          // Card background

// Color Palette - DAY MODE
#define COLOR_BG_MAIN_DAY lv_color_make(240, 245, 250)        // Açık gri-mavi arka plan
#define COLOR_BG_PANEL_DAY lv_color_make(255, 255, 255)       // Beyaz panel
#define COLOR_ACCENT_BLUE_DAY lv_color_make(52, 152, 219)     // Parlak mavi (aktif durum)
#define COLOR_TEXT_PRIMARY_DAY lv_color_make(44, 62, 80)      // Koyu gri-mavi (başlıklar)
#define COLOR_TEXT_SECONDARY_DAY lv_color_make(149, 165, 166) // Orta gri (pasif text)
#define COLOR_INACTIVE_GRAY_DAY lv_color_make(183, 189, 199)  // #b7bdc7 - Pasif slider/buton
#define COLOR_SIDEBAR_BG_DAY lv_color_make(236, 240, 241)     // Açık gri
#define COLOR_CARD_BG_DAY lv_color_make(255, 255, 255)        // Beyaz
#define COLOR_BORDER_DAY lv_color_make(189, 195, 199)         // Açık border
#define COLOR_SELECTED_BG_DAY lv_color_make(225, 236, 244)    // Açık mavi arka plan (seçili)
...
static void update_slider_visual(void)
{
    ...
    int full_segments = slider_percent / 10;  // Tamamen dolu segmentler
    int partial_percent = slider_percent % 10; // Son segmentin doluluk yüzdesi (0-9)
...
void usrToiletPage_updateThemeColors(void) 
{
    ...
    if (is_day_mode)
    {
        ...
        if (right_panel != NULL)
        {
            ...
            // Yüzde yazısı - buton ismiyle aynı renk (mavi)
            lv_obj_set_style_text_color(buttonPage.main_status_label, COLOR_ACCENT_BLUE_DAY, 0);
            
            // Arka plan segmentlerini güncelle (pasif slider)
            for (int i = 0; i < 10; i++)
            {
...
static void main_slider_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED)
    {
        ...
        {
            ESP_LOGW(s_tag, "Slider change ignored - device is OFF");
            return;
        }
        ...
        // Segmentleri güncelle - KISMÎ DOLULUK DESTEĞİ
        int full_segments = value / 10;  // Tamamen dolu segmentler
        int partial_percent = value % 10; // Son segmentin doluluk yüzdesi (0-9)
        
        ESP_LOGI(s_tag, "Slider:  %ld%%, Full segments: %d, Partial: %d%%", 
                 value, full_segments, partial_percent * 10);
        ...
        for (int i = 0; i < 10; i++)
        {
            if (active_segments[i] != NULL)
            {
                ...
                {
                    // Kısmen dolu segment - opacity ile göster
                    lv_obj_clear_flag(active_segments[i], LV_OBJ_FLAG_HIDDEN);
                    // 10-90 arası opacity (her %10'luk artış = 10 opacity)
                    lv_opa_t opacity = (lv_opa_t)((partial_percent * 255) / 10);
                    if (opacity < 30) opacity = 30; // Minimum görünürlük
                    lv_obj_set_style_bg_opa(active_segments[i], opacity, 0);
                    lv_obj_set_style_shadow_opa(active_segments[i], opacity * 70 / 100, 0);
                }
                else
        ...
        // Send slider value via CAN
        uint32_t can_id = CAN_ID_SLIDER_BASE + channel;
        esp_err_t result = sendCanHeader(can_id, sliderPos[channel]);
        if (result == ESP_OK)
        {
            ESP_LOGI(s_tag, "Slider %d value sent:  %lu (ID: 0x%lX)",
                     channel + 1, (unsigned long)sliderPos[channel], (unsigned long)can_id);
        }
    }
}
...
static void main_power_btn_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        if (buttonPage.button_states[channel])
        {
            ...
            sendCanHeader(CAN_ID_SLIDER_BASE + channel, sliderPos[channel]);
        }
        else
        {
            bool is_day_mode = !usrGraphicalInterface_isDarkMode();
            // Turn OFF - DAY/NIGHT MODE AWARE
            ...
            // Tüm segmentleri gizle
            for (int i = 0; i < 10; i++)
            {
                lv_obj_add_flag(active_segments[i], LV_OBJ_FLAG_HIDDEN);
            }
            
            sendCanHeader(CAN_ID_SLIDER_BASE + channel, 0);
        }
        
        // Send button state via CAN
        uint32_t button_value = buttonPage.button_states[channel] ? 1 : 0;
        sendCanHeader(CAN_ID_BUTTON_BASE + channel, button_value);
        
        // SIDEBAR GÖSTERGELERİNİ GÜNCELLE
        update_sidebar_indicators();
...
static void sidebar_btn_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        {
            // Deselect old button - DAY/NIGHT MODE AWARE
            ...
            {
                // Device ON - DAY/NIGHT MODE AWARE
                bool is_day_mode = !usrGraphicalInterface_isDarkMode();
                lv_color_t active_color = is_day_mode ? COLOR_ACCENT_BLUE_DAY : COLOR_NEON_CYAN;
                lv_color_t bg_color = is_day_mode ? lv_color_make(235, 245, 251) : lv_color_make(15, 25, 35);
            ...
            {
                // Device OFF - DAY/NIGHT MODE AWARE
                lv_color_t inactive_color = is_day_mode ? COLOR_INACTIVE_GRAY_DAY : COLOR_TEXT_SECONDARY_NIGHT;
                lv_color_t bg_color = is_day_mode ? lv_color_make(246, 248, 251) : lv_color_make(15, 25, 35);
            ...
            ESP_LOGI(s_tag, "Selected device: %s (index %d)", 
...
static void close_all_btn_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        ESP_LOGI(s_tag, "Close All button clicked - turning off all devices");
        
        // Turn off all devices - SLIDER DEĞERLERİNİ KORUYORUZ
        for (int i = 0; i < 6; i++)
        {
            if (buttonPage.button_states[i])
            {
                ...
                sendCanHeader(CAN_ID_BUTTON_BASE + i, 0);
                sendCanHeader(CAN_ID_SLIDER_BASE + i, 0);
            }
        }
...
void usrToiletPage_init(lv_obj_t *parent)
{
    ...
    {
        ESP_LOGW(s_tag, "Toilet page already initialized");
        return;
    }
    ...
    lv_label_set_text(buttonPage.header_icon, LV_SYMBOL_TINT);
    ...
    lv_label_set_text(buttonPage.title_label, "TOILET");
    ...
    // Select PWM-1 by default (index 0)
    ...
    lv_label_set_text(buttonPage.main_device_name, "LIGHTING CONTROL");
   ...
   lv_obj_t *right_panel = lv_obj_create(buttonPage.page_container);
    ...
    // RIGHT PANEL (SLIDER) - init fonksiyonunda
    // Arka plan segmentleri (koyu, pasif durumlar) - DAY MODE İÇİN
    for (int i = 0; i < SEGMENT_COUNT; i++)
    {
        bg_segments[i] = lv_obj_create(slider_container);  // GLOBAL ARRAY'E KAYDET
    ...
    // Gizli slider (sadece değer kontrolü için)
    ...
    // Aktif segmentler (parlak, açık durumlar) - GLOBAL active_segments dizisini kullan
    for (int i = 0; i < SEGMENT_COUNT; i++)
    {
    ...
    ESP_LOGI(s_tag, "Toilet page initialized");
}
...
void usrToiletPage_show(void)
{
    if (buttonPage.page_container != NULL)
    {
        ...
        ESP_LOGI(s_tag, "Toilet page shown");
    }
}
...
void usrToiletPage_hide(void)
{
    if (buttonPage.page_container != NULL)
    {
        ...
        ESP_LOGI(s_tag, "Toilet page hidden");
    }
}
...
void usrToiletPage_destroy(void)
{
    if (buttonPage.page_container != NULL)
    {
        ...
        memset(&buttonPage, 0, sizeof(usrToiletPage_t));
        ESP_LOGI(s_tag, "Toilet page destroyed");
    }
}
...
{
    ESP_LOGI(s_tag, "Button-1 handler called");
}

void usrToiletPage_button2_handler(void)
{
    ESP_LOGI(s_tag, "Button-2 handler called");
}

void usrToiletPage_button3_handler(void)
{
    ESP_LOGI(s_tag, "Button-3 handler called");
}

void usrToiletPage_button4_handler(void)
{
    ESP_LOGI(s_tag, "Button-4 handler called");
}

void usrToiletPage_button5_handler(void)
{
    ESP_LOGI(s_tag, "Button-5 handler called");
}

void usrToiletPage_button6_handler(void)
{
    ESP_LOGI(s_tag, "Button-6 handler called");
}
