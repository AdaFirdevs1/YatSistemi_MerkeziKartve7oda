...
{
    // Merkezi kartta node ID değil, room ID döndürülür
    ...
    return selected_room;  // Room ID döndür, aynı getLEDNodeID() gibi
}
...
static void main_power_btn_cb(lv_event_t *e)
{
    ...
    // CENTRAL MASTER FONKSİYONU ÇAĞIR
    sendRelayControlToRoom(
        room_id,
        channel + 1,  // Relay channel (1-6)
        buttonPage.button_states[channel]
    );
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        uint8_t room_id = usrGraphicalInterface_getSelectedRoom();
        if (room_id == 0 || !isRoomActive(room_id)) {
            ESP_LOGE(s_tag, "Room not available!");
            return;
        }
        uint8_t relay_ch = channel + 1;
        sendRelayControlToRoom(room_id, relay_ch, buttonPage.button_states[channel]);
