...
{
    return getNodeIDByType(NODE_TYPE_LED);
}
...
static void main_power_btn_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        // CAN bus communication - LED control
        uint32_t led_node_id = getLEDNodeID();
        ...
        uint8_t led_cmd[8] = {0};
        led_cmd[0] = LED_CMD_SET;
        led_cmd[1] = channel + 1;
        led_cmd[2] = buttonPage.button_states[channel] ? 1 : 0;
        
        esp_err_t err = sendCanMessage(led_node_id, led_cmd, 3);
        if (err == ESP_OK) {
            ESP_LOGI(s_tag, "✓ LED%d SET sent to 0x%03lX:  State=%d", 
                     channel+1, led_node_id, buttonPage.button_states[channel]);
        } else {
            ESP_LOGE(s_tag, "✗ LED%d SET FAILED", channel+1);
            return;
        }
        ...
        if (buttonPage.button_states[channel]) {
            ...
            memset(led_cmd, 0, 8);
            led_cmd[0] = LED_CMD_SET_BRIGHTNESS;
            led_cmd[1] = channel + 1;
            led_cmd[2] = (uint8_t)(sliderPos[channel] >> 8);
            led_cmd[3] = (uint8_t)(sliderPos[channel] & 0xFF);
...
static void close_all_btn_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ESP_LOGI(s_tag, "Close All button clicked - turning off all LEDs");
        ...
        // Önce tüm LED'leri kapat (çok hızlı ard arda gönder)
        for (int i = 0; i < 6; i++)
        {
            if (buttonPage.button_states[i])
            {
                // Sadece button state'i güncelle
                buttonPage.button_states[i] = false;
                
                // LED'i kapat - GECİKMESİZ GÖNDER
                uint8_t led_cmd[8] = {0};
                led_cmd[0] = LED_CMD_SET;
                led_cmd[1] = i + 1;
                led_cmd[2] = 0;  // OFF
                ...
                esp_err_t err = sendCanMessage(led_node_id, led_cmd, 3);
                if (err == ESP_OK) {
                    ESP_LOGI(s_tag, "✓ LED%d SET=OFF sent (brightness %lu korundu)", 
                             i+1, sliderPos[i]);
                } else {
                    ESP_LOGE(s_tag, "✗ LED%d SET FAILED", i+1);
                }
                
            }
        }
        
        // UI güncelleme - Sadece görünümü güncelle
        int channel = buttonPage.selected_index;
        lv_obj_t *power_icon = lv_obj_get_child(buttonPage.main_power_btn, 0);
        ...
        // Slider'ı devre dışı bırak AMA DEĞERİNİ KORU
        lv_obj_add_state(buttonPage.main_slider, LV_STATE_DISABLED);
        lv_obj_clear_flag(buttonPage.main_slider, LV_OBJ_FLAG_CLICKABLE);
        
        // YÜZDE GÖSTERGESİNİ SIFIRLA (ama slider değerini koru)
        lv_label_set_text(buttonPage. main_status_label, "0%");
        
        // SLIDER GÖRSELİNİ SIFIRLA (ama sliderPos[] değişmeyecek)
        lv_slider_set_value(buttonPage. main_slider, 0, LV_ANIM_OFF);
        
        // Segmentleri gizle (slider değeri hala bellekte)
        for (int i = 0; i < 10; i++)
        {
            if (active_segments[i] != NULL)
            {
        ...
        ESP_LOGI(s_tag, "All LEDs turned OFF instantly - brightness values preserved in memory");
    }
}
